"""
Представление страницы «Удаление альбома» по адресу /music/del/album/<int:_id>/.
"""
...
def form_del_album(request, _id: int):
    ...
    if len(cnt):
        ...
        django_pager = get_django_pager(get_show_del_query(5, tf, _id), request)
        records = []
        ...
        rel_data.append({
            ...
            'th': [django_head_sort_link(request, django_pager.getParameters(), f) for f in tf
                   if f[0] != tf[cnt[0]][0]],
            'records': records,
            'isp': django_pager,
