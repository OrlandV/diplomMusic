...
from .show_author_query import get_show_author_query
...
def show(request, cf: list, set_ref: str | bool, err: list | None = None) -> HttpResponse:
    af = cur_fields([0])
    af.extend(cf)
    sr = ShowResult(get_show_author_query(af, request.GET), request)
    if items := sr.getISP().getItems(request.GET):
        ...
        for record in items:
            ...
            ed = [
                f'<a href="/music/edit/{fields()[24][2]}/{record[0]}/"><img src="/static/img/pencil.png" /></a>',
                f'<a href="/music/del/{fields()[24][2]}/{record[0]}/" class="adel" style="color: red;">×</a>'
            ]
            temp.append(ed)
            records.append(temp)
    else:
        records = False
    templ_name = f'add_{fields()[24][2]}.html'
    context = {
